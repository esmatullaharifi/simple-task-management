                        ...
                        <form method="POST" action="{{ route('tasks.update', $task->id) }}">
                            ...
                            <div style="padding-bottom: 15px;">
                                ...
                                <select id="project_id"
                                        ...
                                        name="project_id" required autofocus>
                                    @foreach($projects as $project)
                                        <option value="{{ $project->id }}"
                                                @if($project->id == (old('project_id') ?? $task->project_id)) selected @endif>{{ $project->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="mt-4">
                                ...
                                <x-input id="priority" class="block mt-1 w-full" type="number" name="priority" min="1"
                                         step="1" :value="old('priority') ?? $task->priority" required/>
                            </div>
